...
def get_active_user_emails():
    """Return list of all active users' valid email addresses."""
    User = get_user_model()
    # Filter for active users and exclude null/empty emails
    all_emails = User.objects.filter(
        is_active=True
    ).exclude(
        email__isnull=True
    ).exclude(
    ...
    # Keep only valid email addresses (must contain @)
    valid_emails = [e for e in all_emails if "@" in e]
    return valid_emails
...
def auto_generate_recurring_meetings():
    ...
    past_meetings = Reunion.objects.filter(
        reminder_sent=True,
        date__lte=now.date()  # Include today's meetings and past meetings
    )
