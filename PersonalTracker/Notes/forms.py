...
class NoteForm(forms.ModelForm):
    tags = forms.ModelMultipleChoiceField(queryset=Tag.objects.none(), label="Tags", required=False)
    class Meta:
        model = Note
...
class SearchTagForm(forms.ModelForm):
    tags = forms.ModelChoiceField(queryset=Tag.objects.none(), label = False)
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        # Filtre dynamique en fonction de l'utilisateur connecté
        if user is not None:
            self.fields['tags'].queryset = Tag.objects.filter(user=user)